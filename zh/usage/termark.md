...
- 如果使用直连代理服务，Termark 还需要能访问 NextTerminal 的 SSH 代理服务器监听地址。
- 如果访问 RDP 资产，Termark 所在机器还需要能访问 NextTerminal 的 RDP 代理服务器对外地址。
...
接下来根据 SSH 资产的访问网络选择连接方式。RDP 资产使用 RDP 代理服务器的对外访问地址，不受这里的 SSH 连接方式影响。

## 3. 选择 SSH 连接方式

这一项用于决定 Termark 如何连接 NextTerminal SSH 代理服务器，只影响 SSH 资产访问。
...
如果 Termark 所在网络可以直接访问 NextTerminal 的 SSH 代理服务器，可以选择「直连代理服务」。直连方式比 WebSocket 隧道少一次转发，延迟更低，交互会更顺畅。

使用直连代理服务前，需要确认 SSH 代理服务器监听地址对 Termark 可达：
...
::: warning 安全建议
如果把 SSH 代理服务器监听地址改为 `0.0.0.0:2022`，表示 SSH 代理端口可能被外部网络访问。请同时配置防火墙、安全组或访问控制策略，只允许可信来源访问该端口。
:::
...
- **SSH 资产**：点击资产后，Termark 通过配置的 SSH 连接方式接入 NextTerminal SSH 代理服务器，再连接目标 SSH 资产。
- **RDP 资产**：点击资产后，Termark 通过 NextTerminal RDP 代理服务器生成短期票据，并调用本地 RDP 客户端连接目标 RDP 资产。
...
1. RDP 代理服务器是否已开启，且 `docker-compose.yaml` 中已映射 RDP 代理端口。
2. RDP 代理服务器「对外访问地址」是否填写为 Termark 所在机器可访问的地址。
3. 防火墙或云服务器安全组是否放行 RDP 代理端口，例如默认的 `3390`。
4. 目标资产协议是否为 `RDP`，资产账号、密码、域和端口是否配置正确。
5. 当前账号是否拥有该 RDP 资产的访问权限。
